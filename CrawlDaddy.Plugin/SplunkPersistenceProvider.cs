...
{
    public class SplunkPersistenceProvider : IPersistenceProvider
    {
        static ILog _splunkLogger = LogManager.GetLogger("SplunkLogger");

        public SplunkPersistenceProvider()
        {
        }

        public SplunkPersistenceProvider(ILog logger)
        {
            if (logger != null)
                _splunkLogger = logger;
        }
