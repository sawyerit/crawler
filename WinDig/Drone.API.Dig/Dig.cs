...
namespace Drone.API.Dig
{
	...
	public class Dig : IDisposable
	{
		...
		private static volatile Dig instance;
		...
		{
			SSLCert myCert = new SSLCert { Issuer = "None", Subject = "None", DomainToCheck = domainName };
        ...
        //internal RecordA GetARecord(string domainName)
        //{
        //    try
        //    {
        //        Response resp = _resolver.Query(domainName, QType.A, QClass.IN);
        //        if (!Object.Equals(resp, null) && resp.RecordsA.Count() > 0)
        //        {
        //            return resp.RecordsA[0] as RecordA;
        //        }
        //    }
        //    catch (Exception e)
        //    {
        ...
        internal ARecord GetARecord(string domainName)
        {
            try
            {
                ...
                if (!Object.Equals(answer, null) && answer.Answers.Count() > 0)
                {
                    ...
                    {
                        if (Object.Equals(null, item.RecType))
                        {
                            ExceptionExtensions.LogError(new ArgumentNullException("item.RecType is NULL"), "Dig.GetARecord", "item: " + item.ToString());
                            return null;
                        }

                        if (item.RecType == RecordType.A && !Object.Equals(null, item.Data))
                        {
        ...
        //internal List<RecordMX> GetMXRecord(string domainName)
        //{
        //    List<RecordMX> records = new List<RecordMX>();
        //    try
        //    {
        //        Response resp = _resolver.Query(domainName, QType.MX, QClass.IN);

        //        if (resp.Answers.Count > 0)
        //            records = resp.Answers.Select(rec => rec.RECORD as RecordMX).ToList();
        //    }
        //    catch (Exception e)
        //    {
